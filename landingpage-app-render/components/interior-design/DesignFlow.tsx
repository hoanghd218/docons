...
/* ── Daily render limit (localStorage) ─────────────────────── */
const MAX_RENDERS_PER_DAY = 3;
const STORAGE_KEY = 'docons_render_usage';
...
function incrementRenderCount(): void {
...
function isLimitReached(): boolean {
    return getRenderCount() >= MAX_RENDERS_PER_DAY;
}
/* ──────────────────────────────────────────────────────────── */
...
const DesignFlow: React.FC<DesignFlowProps> = ({ onComplete }) => {
    ...
    const [remainingRenders, setRemainingRenders] = useState(MAX_RENDERS_PER_DAY);

    const resultsRef = useRef<HTMLDivElement>(null);
    const editAreaRef = useRef<HTMLDivElement>(null);

    // Sync remaining renders on mount
    React.useEffect(() => {
        setRemainingRenders(MAX_RENDERS_PER_DAY - getRenderCount());
    }, []);

    const checkAndConsumeRender = useCallback((): boolean => {
        if (isLimitReached()) {
                ...
                {generatedImages.map((imgUrl, idx) => (
                    <div key={idx} className="flex flex-col group animate-scaleIn" style={{ animationDelay: `${idx * 0.1}s` }}>
                        ...
                        <div className="text-center">
                            ...
                            <a
                                href="https://zalo.me/g/kjykoa854"
                                target="_blank"
    ...
    return (
        <section className="min-h-screen pt-24 pb-20 relative bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50">
            ...
            <div className="container mx-auto px-4 mb-12">
                <div className="max-w-6xl mx-auto glass-liquid rounded-3xl shadow-2xl shadow-primary/5 border border-white/40 overflow-hidden flex flex-col lg:flex-row animate-fadeIn" style={{ animationDelay: '0.2s' }}>
                    ...
                    <div className="w-full lg:w-5/12 p-6 md:p-10 bg-white flex flex-col h-full">
                        ...
                        <div className="mt-auto pt-6 border-t border-slate-100">
                            ...
                            {remainingRenders <= MAX_RENDERS_PER_DAY && (
                                <p className={`text-xs text-center mt-3 font-medium ${remainingRenders === 0 ? 'text-red-500' : remainingRenders === 1 ? 'text-orange-500' : 'text-slate-400'
                                    }`}>
                                    {remainingRenders > 0
                                        ? `Còn ${remainingRenders}/${MAX_RENDERS_PER_DAY} lượt thiết kế AI hôm nay`
                                        : 'Đã hết lượt thiết kế AI hôm nay'
                                    }
                                </p>
                            )}
                        </div>
                ...
                <div className="bg-gradient-to-r from-primary to-primary-hover rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 text-white shadow-2xl shadow-primary/30 relative overflow-hidden animate-fadeIn">
                    ...
                    <div className="relative z-10 flex-1 text-center md:text-left">
                        <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
                            ...
                            <h3 className="text-2xl md:text-3xl font-bold font-display">Cộng đồng nhà đẹp Tây Ninh</h3>
                        </div>
            ...
            {showLimitModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="relative w-full max-w-md glass-strong rounded-3xl shadow-2xl overflow-hidden border border-white/40">
                        ...
                        <div className="bg-gradient-to-r from-accent to-accent-hover h-1.5 w-full"></div>

                        <div className="p-8 text-center">
                            {/* Icon */}
                            <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-accent/10 to-accent/5 flex items-center justify-center shadow-lg shadow-accent/10">
                                <SparklesIcon size={40} className="text-accent" />
                            </div>

                            <h3 className="text-2xl font-bold text-slate-900 font-display mb-3">
                                Đã hết lượt thiết kế <span className="gradient-text-accent">AI miễn phí</span> hôm nay!
                            </h3>
                            <p className="text-slate-500 mb-8 leading-relaxed">
                                Bạn đã sử dụng hết <strong className="text-slate-700">{MAX_RENDERS_PER_DAY} lượt</strong> thiết kế AI trong ngày.
                                Hãy liên hệ kiến trúc sư của <strong className="text-primary">DOCONS</strong> để được tư vấn miễn phí nhé!
                            </p>
                            ...
                            <div className="space-y-3">
                                ...
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="w-full py-4 bg-gradient-to-r from-primary to-primary-hover text-white rounded-xl font-bold shadow-lg shadow-primary/30 transition-all flex items-center justify-center gap-3 hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 group"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="group-hover:rotate-12 transition-transform">
                                        <path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z" />
                                    </svg>
                                    Tham gia nhóm Zalo tư vấn
                                </a>

                                {/* Phone Call CTA */}
                                <a
                                    href="tel:0345406157"
                                    className="w-full py-4 bg-white border-2 border-primary/20 text-primary rounded-xl font-bold transition-all flex items-center justify-center gap-3 hover:bg-primary/5 hover:border-primary/40 hover:-translate-y-0.5 active:translate-y-0 group"
                                >
                                    ...
                                    Gọi ngay: 0345 406 157
                                </a>
                            </div>

                            <p className="text-xs text-slate-400 mt-6">
                                Lượt thiết kế AI sẽ được làm mới vào ngày mai.
                            </p>
                        </div>
                    </div>
                </div>
